...
public class Protocol {
	...
	public static void send(byte[] bytes) throws IOException, NullPointerException {
		...
		for (int i = 1; i < bytes.length; i++) {
		    if (bytes[i] == Message.start || bytes[i] == Message.DataLinkEscape) {
		        byteArrayOutputStream.write(Message.DataLinkEscape);
		    }
		    byteArrayOutputStream.write(bytes[i]);
		}

		byte[] crcBytes = crc(bytes);
		if (crcBytes[0] == Message.start || crcBytes[0] == Message.DataLinkEscape) {
		    byteArrayOutputStream.write(Message.DataLinkEscape);
		}
		byteArrayOutputStream.write(crcBytes[0]);

		if (crcBytes[1] == Message.start || crcBytes[1] == Message.DataLinkEscape) {
		    byteArrayOutputStream.write(Message.DataLinkEscape);
		}
		byteArrayOutputStream.write(crcBytes[1]);
